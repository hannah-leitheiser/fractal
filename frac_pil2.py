...
def Mandlebrot(x, y, maxIterations=32):
	...
	for i in range(maxIterations):
		z = z**2 + c
		if abs(z.imag) > 2 or abs(z.real) > 2:
			return i+1
		if maskpixels[ int((z.real+2) * (1023/4)), int(((z.imag+2)) * (782/4))][0] == 255:
			return i+1
	return 0
...
for x in range(imageSize[0]):
	for y in range(imageSize[1]):
		...
		pixels[x,y] = (Mandlebrot(cordx, cordy,32)*8,0,0)
